...
repeat.experiment = data.frame(matrix(NA,3900,6))
colnames(repeat.experiment) = c("rep", "Lambda", "Side", "CPU_Time", "CPU_Lambda", "CPU_Intercept" )            # 
          ...
          system.time({
            ...
            for (j in 4:29){
              time_side = 
                ...
                #  sys.g <- system.time({res.g = torch_car(y,D, device = 2 , epochs = Epochs , learningrate = learn ) })  # device 0,1,2 = GPU else CPU })
                #  lam.g <- res.g$lambda
                #  int.g <- res.g$MT
                  ...
                  repeat.experiment[counter, 4] <- sys.c[3] 
                  repeat.experiment[counter, 5] <- lam.c
                  repeat.experiment[counter, 6] <- int.c
                  ...
                  #XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
                  saveRDS(repeat.experiment, file = "CPU")
                }
                )} 

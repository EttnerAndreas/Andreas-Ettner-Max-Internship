...
colnames(repeat.experiment) = c("rep", "Lambda", "Side", "GPU_Time", "GPU_Lambda", "GPU_Intercept" )            # 
            ...
            for (j in 2:11){
              time_side = 
               ...
               #   side = 2 + 2*j
                  ...
                  if (l.r > 2.4 ){    # (l.r > 2.4 ) epochs = 800L, learn = 0.06 / cut GPU1
                    Epochs = 1100L
                  ...
                  sys.g <- system.time({res.g = torch_car(y,D, device = 1 , epochs = Epochs , learningrate = learn ) })  # device 0,1,2 = GPU else CPU })
                  lam.g <- res.g$lambda
                  int.g <- res.g$MT
                  ...
                  #XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
                  saveRDS(repeat.experiment, file = "GPU.new")
                }
                )} 
